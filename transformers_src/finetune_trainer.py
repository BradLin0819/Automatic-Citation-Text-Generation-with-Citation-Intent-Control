...
# This code is adapted from https://github.com/huggingface/transformers/blob/v4.2.2-patch/examples/seq2seq/finetune_trainer.py
...
class CallBackArguments:
    """
    Arguments for callback function such as early stopping.
    """

    early_stopping_patience: Optional[int] = field(
        default=None, metadata={"help": "The number of times validation metrics failed to improve."}
    )
...
def main():
    ...
    parser = HfArgumentParser(
        (ModelArguments, DataTrainingArguments, Seq2SeqTrainingArguments, CallBackArguments))

    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argument to the script and it's the path to a json file,
        # let's parse it to get our arguments.
        model_args, data_args, training_args, callback_args = parser.parse_json_file(
    ...
    if callback_args.early_stopping_patience:
        trainer.add_callback(EarlyStoppingCallback(
            early_stopping_patience=callback_args.early_stopping_patience))
